...
import DivineForce.Spell;
import DivineForce.SpellTest1;
import DivineForce.SpellTest2;
import DivineForce.SpellTest3;
...
public abstract class EntityPlayer extends EntityLivingBase implements ICommandSender
{
    ...
    public float criticalChance = 35;
    public float attackDamage;
    public float totalDamage;
    public float dps;
    public float dpsTimer;
    public float dpsResetTimer;
    public EntityLivingBase target;
    public Container spellContainer;
    public Spell[] spellList = new Spell[20];
    public Spell[] spellBar = new Spell[7];
    ...
    {
    	this.spellKeyBindings();
//    	if(this.getLookedEntity() != null)
  //  	this.addChatMessage(this.getLookedEntity()+" " + this.getLookedEntity().stun + " " + worldObj.isRemote);
    	this.spellList[0] = new SpellTest1();
    	this.spellList[1] = new SpellTest2();
    	this.spellList[2] = new SpellTest3();
    	if(this.dpsResetTimer >= 0)
    	{
    		this.dpsResetTimer ++;
    		this.dpsTimer++;
    		if(this.dpsTimer % 20 == 0)
    		{
    			this.dps = totalDamage/(dpsTimer/20);
    		}
    	}
    	if(this.dpsResetTimer > 100)
    	{
    		this.dpsTimer = 0;
    		this.dps = 0;
    		this.totalDamage = 0;
    		this.dpsResetTimer = -1;
    	}
    	...
    	{ 		
    		this.attackDamage = 1 + this.getHeldItem().getAttackDamage() + this.getTotalAttackDamageFromArmor();
    		this.attackSpeed = ((int)16 /(1 + this.getHaste() / 100));
    		this.criticalChance = inventory.getCurrentItem().getCrit() + this.getTotalCriticalChanceFromArmor();
    	}
...
//		]        	
    ...
    public void attackTargetEntityWithCurrentItem(Entity par1Entity)
    {
        ...
        if (par1Entity.canAttackWithItem())
        {
            if (!par1Entity.hitByEntity(this))
            {
            	//moded
                float f = 0;
            	if(this.getSpecialAttackDamage() <= 0)
            	{
            		f = (float)this.getEntityAttribute(SharedMonsterAttributes.attackDamage).getAttributeValue() + attackDamage;
            	}else
            	{
                	f = this.getSpecialAttackDamage();
            	}
                if(f > 1)
                {
                	f += f*rand.nextDouble()/10  - f*rand.nextDouble()/10;
                }
               	int i = 0;
                float f1 = 0.0F;
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    float randCrit = rand.nextFloat()*100;
                    
                    if (randCrit < this.criticalChance)
                    {
                        f *= 2F;
                    }
                    ...
                    if (flag2)
                    {
                        ...
                        if (randCrit < this.criticalChance)
                        {
                            ...
                            ((EntityLivingBase)par1Entity).splashDamage(f, this.getSpecialAttackDamage() > 0, true);
                        }else
                        {
                        	((EntityLivingBase)par1Entity).splashDamage(f, this.getSpecialAttackDamage() > 0, false);
                        }
    ...
    public float getTotalCriticalChanceFromArmor()
    {
        float crit = 0;
        for(int j = 0; j < inventory.armorInventory.length; j++)
        {
            ...
            {
            	ItemStack itemstack = inventory.armorInventory[j];
                float f = itemstack.getCrit();
                crit += f;
            }
        }
        return crit;
    ...
    public float getTotalAttackSpeedFromArmor()
    {
        float speed = 0;
        for(int j = 0; j < inventory.armorInventory.length; j++)
        {
            ...
            {
            	ItemStack itemstack = inventory.armorInventory[j];
                float f = itemstack.getAttackSpeed();
                speed += f;
            }
        }
        return speed;
    ...
    public float getTotalAttackDamageFromArmor()
    {
        float attack = 0;
        for(int j = 0; j < inventory.armorInventory.length; j++)
        {
            ...
            {
            	ItemStack itemstack = inventory.armorInventory[j];
                float f = itemstack.getAttackDamage();
                attack += f;
            }
        }
        return attack;
    ...
    public float getHaste()
    {
    	...
    	{
    		return inventory.getCurrentItem().getAttackSpeed() + this.getTotalAttackSpeedFromArmor();
    	}
    }
    ...
    public void setSpecialAttackDamage(int par1)
    {
        this.teest = par1;
    }
    
    public void spellKeyBindings()
    {
    	if(Keyboard.isKeyDown(Keyboard.KEY_A))
    	{
    		if(this.spellBar[0] != null) this.spellBar[0].triggerSpell(this.username);
    	}
    	if(Keyboard.isKeyDown(Keyboard.KEY_R))
    	{
    		if(this.spellBar[1] != null) this.spellBar[1].triggerSpell(this.username);
    	}
    	if(Keyboard.isKeyDown(Keyboard.KEY_F))
    	{
    		if(this.spellBar[2] != null) this.spellBar[2].triggerSpell(this.username);
    	}
    	if(Keyboard.isKeyDown(Keyboard.KEY_X))
    	{
    		if(this.spellBar[3] != null) this.spellBar[3].triggerSpell(this.username);
    	}
    	if(Keyboard.isKeyDown(Keyboard.KEY_V))
    	{
    		if(this.spellBar[4] != null) this.spellBar[4].triggerSpell(this.username);
    	}
    	if(Keyboard.isKeyDown(Keyboard.KEY_B))
    	{
    		Minecraft.getMinecraft().displayGuiScreen(new GuiSpells(this));
    	}
    }
    
    public EntityLivingBase getLookedEntity()
    {
        ...
        {		
			return (EntityLivingBase) Minecraft.getMinecraft().objectMouseOver.entityHit;
        } else
        {
        	return null;
        }
