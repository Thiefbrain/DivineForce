...
public abstract class RendererLivingEntity extends Render
{
    ...
    public void doRenderLiving(EntityLivingBase par1EntityLivingBase, double par2, double par4, double par6, float par8, float par9)
    {
        ...
        //moded
        this.renderHealthBar(par1EntityLivingBase, par2, par4, par6);
        OpenGlHelper.setActiveTexture(OpenGlHelper.lightmapTexUnit);
        ...
        try
        {
            ...
            if ((i >> 24 & 255) > 0 || par1EntityLivingBase.hurtTime > 0 || par1EntityLivingBase.deathTime > 0)
            {
                ...
                {
                	//moded
                    GL11.glColor4f(f14, 0.0F, 0.0F, 0.15F);
            ...
            //moded
            if (par1EntityLivingBase.isStuned())
            {
                GL11.glDisable(GL11.GL_TEXTURE_2D);
                GL11.glDisable(GL11.GL_ALPHA_TEST);
                GL11.glEnable(GL11.GL_BLEND);
                GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
                GL11.glDepthFunc(GL11.GL_EQUAL);
                GL11.glColor4f(1, 1.0F, 1.0F, 0.15F);
                this.mainModel.render(par1EntityLivingBase, f8, f7, f4, f3 - f2, f5, f6);
                for (int l = 0; l < 4; ++l)
                {
                    if (this.inheritRenderPass(par1EntityLivingBase, l, par9) >= 0)
    ...
    protected void renderHealthBar(EntityLivingBase par1EntityLivingBase, double par2, double par4, double par6)
    {
    	double distance = par1EntityLivingBase.getDistanceToEntity(this.renderManager.livingPlayer);
        if(distance < 12 && par1EntityLivingBase.getHealth() > 0)
        {
            float f = 1.6F;
            float f1 = 0.016666668F * f;
	        GL11.glPushMatrix();
	        GL11.glTranslatef((float)par2 + 0.0F, (float)par4 + par1EntityLivingBase.height + 0.65F, (float)par6);
	        GL11.glRotatef(-this.renderManager.playerViewY, 0.0F, 1.0F, 0.0F);
	        GL11.glRotatef(this.renderManager.playerViewX, 1.0F, 0.0F, 0.0F);
	        GL11.glScalef(-f1, -f1, f1);
	        GL11.glDisable(GL11.GL_LIGHTING);
	        GL11.glTranslatef(0.0F, 0.25F / f1, 0.0F);	        
	        GL11.glDisable(GL11.GL_TEXTURE_2D);
	        float i = -12.5F + par1EntityLivingBase.getHealthPercentage()/4F;
	        float i2 = -12.5F + par1EntityLivingBase.damageBarAnim/4F;
	        Tessellator tessellator = Tessellator.instance;
	        tessellator.startDrawingQuads();
	        tessellator.setColorRGBA_F(0.35F,0.0F,0.0F,1F);
	        tessellator.addVertex((double)(12.5), -1.0D, 0.0D);
	        tessellator.addVertex((double)(12.5), 0.0D, 0.0D);
	        tessellator.addVertex((double)(i2), 0.0D, 0.0D);
	        tessellator.addVertex((double)(i2), -1.0D, 0.0D);
	        tessellator.draw();
	        Tessellator tessellator2 = Tessellator.instance;
	        tessellator2.startDrawingQuads();        
	        tessellator2.setColorRGBA_F(0.8F + (float)par1EntityLivingBase.hurtTime/10, 0.1F + (float)par1EntityLivingBase.hurtTime/10, 0.1F + (float)par1EntityLivingBase.hurtTime/10, 1.0F);
	        tessellator2.addVertex((double)(-12.5), -1.0D, 0.0D);
	        tessellator2.addVertex((double)(-12.5), 0.0D, 0.0D);
	        tessellator2.addVertex((double)(i), 0.0D, 0.0D);
	        tessellator2.addVertex((double)(i), -1.0D, 0.0D);
	        tessellator2.draw();
	        Tessellator tessellator3 = Tessellator.instance;
	        tessellator3.startDrawingQuads();
	        tessellator3.setColorRGBA_F(1,1,1,1);
	        tessellator3.addVertex((double)(i), -1.0D, 0.0D);
	        tessellator3.addVertex((double)(i), 0.0D, 0.0D);
	        tessellator3.addVertex((double)(i2), 0.0D, 0.0D);
	        tessellator3.addVertex((double)(i2), -1.0D, 0.0D);
	        tessellator3.draw();
	        GL11.glEnable(GL11.GL_TEXTURE_2D);
	        GL11.glDepthMask(true);
	        String s = par1EntityLivingBase.getTranslatedEntityName() + " Lvl " + par1EntityLivingBase.getLevel();
     
	        FontRenderer fontrenderer = this.getFontRendererFromRenderManager();
            int color = 0xffffff;
            switch (par1EntityLivingBase.getRarity())
            {
            	case 1 : color = 0xffffff; break;
            	case 2 : color = 0x33ff11; break;
            	case 3 : color = 0x2288cc; break;
            	case 4 : color = 0xcc00ff; break;
            	case 5 : color = 0xffff55; break;
            }
            f1 = -0.4f;
	        GL11.glScalef(-f1, -f1, f1);
            fontrenderer.drawString(s,-fontrenderer.getStringWidth(s) / 2, -18, color);
	        GL11.glEnable(GL11.GL_LIGHTING);
	        GL11.glDisable(GL11.GL_BLEND);
	        GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
	        GL11.glPopMatrix();
