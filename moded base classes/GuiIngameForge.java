...
public class GuiIngameForge extends GuiIngame
{
    ...
    private static final ResourceLocation MOB_UI = new ResourceLocation("textures/gui/hopper.png");
    ...
    protected void renderHUDText(int width, int height)
    {
        ...
        //moded
        if(mc.thePlayer != null && mc.thePlayer.target != null)
        {
        	 renderMobUI(width, height);
        }
//        renderDPS();
        renderSpellBar();
    ...
    protected void renderMobUI(int width, int height)
    {
        GL11.glEnable(GL11.GL_BLEND);
    	EntityLivingBase theTarget = Minecraft.getMinecraft().thePlayer.target;
    	String targetName = mc.thePlayer.target.getEntityName();
    	String health = "";
    	String level = "" + 1;
    	int color = 0xffffff;
		level =	"" + theTarget.getLevel();
    	health = ((int)Math.ceil(theTarget.getHealth())) + "/" +  (int)Math.ceil(theTarget.getMaxHealth());
		if(theTarget.getRarity() == 2)
		{
			color = 0x33ff11;
		}
		if(theTarget.getRarity() == 3)
		{
			color = 0x11aaff;
		} 		
		if(theTarget.getRarity() == 4)
		{
			color = 0xcc00ff;
		}   	
		if(theTarget.getRarity() == 5)
		{
			color = 0xffff55;
		} 
    	fontrenderer.drawStringWithShadow(targetName, 72 - fontrenderer.getStringWidth(targetName) / 2, 8, color);
  	 	fontrenderer.drawStringWithShadow("Lvl", 8,9, 0xffaa00);
  	 	fontrenderer.drawStringWithShadow(level, 15 - fontrenderer.getStringWidth(level)/2,21, 0xffff55);
  	 	fontrenderer.drawString(health, 72 - fontrenderer.getStringWidth(health)/2, 21, 0xffffff);
    	this.mc.getTextureManager().bindTexture(new ResourceLocation("divineforceassets","textures/guis/gui.png"));
        this.drawTexturedModalRect(5, 5, 0, 0, 122, 26);
        this.drawTexturedModalRect(25, 20, 20, 26, (int)theTarget.getHealthPercentage(), 12);
        GL11.glDisable(GL11.GL_BLEND);
    }
    
    protected void renderDPS()
    {
  	 	fontrenderer.drawString("DPS : " + Minecraft.getMinecraft().thePlayer.dps, 80, 50, 0xffffff);	
    }
    
    protected void renderSpellBar()
    {
        GL11.glEnable(GL11.GL_BLEND);
    	EntityPlayer player = Minecraft.getMinecraft().thePlayer;
        int width = res.getScaledWidth();
        int height = res.getScaledHeight();
    	this.mc.getTextureManager().bindTexture(new ResourceLocation("divineforceassets","textures/guis/guispellinventory.png"));
        this.drawTexturedModalRect(width - 28, height/2 - 51, 176, 0, 22, 142);
    	this.mc.getTextureManager().bindTexture(new ResourceLocation("divineforceassets","textures/guis/guispells.png"));
        for(int i = 0; i < 7; i++)
        {
        	if(player.spellBar[i] != null)
        	{
        		this.drawTexturedModalRect(width - 25, height/2 - 48 + i*20, player.spellBar[i].iconX()*16, player.spellBar[i].iconY(), 16, 16);
        		if(player.spellBar[i].cooldownTimer > 0)
        		{
        	    	this.mc.getTextureManager().bindTexture(new ResourceLocation("divineforceassets","textures/guis/guispellinventory.png"));
            		this.drawTexturedModalRect(width - 25, height/2 - 48 + i*20, 176, 142, (int)Math.ceil(player.spellBar[i].getCooldownForRender()), 16);    			
                	this.mc.getTextureManager().bindTexture(new ResourceLocation("divineforceassets","textures/guis/guispells.png"));
        		}
        	}
        }
