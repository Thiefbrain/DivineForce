...
import DivineForce.EntityDamageFX;
import DivineForce.DivineForce;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    private int weaponRarityRand;
    public int stun;
    public float damageBarAnim;
    ...
    public EntityLivingBase(World par1World)
    {
        ...
        {
	        int rarityRand = rand.nextInt(100);
	        ...
	        if(rarityRand > 60 && rarityRand < 85)
	        {
	        	setRarity(2);
	        } 
	        else
	        if(rarityRand > 85 && rarityRand < 95)
	        {
	        	setRarity(3);
	        } 
	        else
    ...
    protected void entityInit()
    {
        ...
        //moded
        this.dataWatcher.addObject(20, Integer.valueOf(1)); // level
		this.dataWatcher.addObject(21, Integer.valueOf(1)); // rarity
		this.dataWatcher.addObject(22, Integer.valueOf(0)); // stun
    }
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                    	Minecraft.getMinecraft().thePlayer.dpsResetTimer = 0;
                    	Minecraft.getMinecraft().thePlayer.totalDamage += Math.round(par2);
                    }
                }
    ...
    public void onDeath(DamageSource par1DamageSource)
    {
        ...
        {
	    	switch (rand.nextInt(this.getRarity() + 1))
	    	{
	    		case 1 : break;
	    		case 2 :
	    		{
	    			this.dropRandomSword(Item.bootsDiamond.itemID, 1, 4);
	    			this.dropRandomSword(Item.legsDiamond.itemID, 1, 4);
	    			this.dropRandomSword(Item.helmetDiamond.itemID, 1, 4);
	    			this.dropRandomSword(Item.plateDiamond.itemID, 1, 4);
	    			switch (rand.nextInt(4))
	    			{
	    				case 0 : this.dropRandomSword(DivineForce.commonSword1.itemID, 1, 2);break;
	    				case 1 : this.dropRandomSword(DivineForce.commonSword2.itemID, 1, 2);break;
	    				case 2 : this.dropRandomSword(DivineForce.commonSword3.itemID, 1, 2);break;
	    				case 3 : this.dropRandomSword(DivineForce.commonSword4.itemID, 1, 2);break;
	    			}			
	    		}break;
	    		case 3 :
	    		{
	    			switch (rand.nextInt(4))
	    			{
	    				case 0 : this.dropRandomSword(DivineForce.rareSword1.itemID, 1, 3);break;
	    				case 1 : this.dropRandomSword(DivineForce.rareSword2.itemID, 1, 3);break;
	    				case 2 : this.dropRandomSword(DivineForce.rareSword3.itemID, 1, 3);break;
	    				case 3 : this.dropRandomSword(DivineForce.rareSword4.itemID, 1, 3);break;
	    			}			
	    		}break;
	    		case 4 :
	    		{
	    			switch (rand.nextInt(4))
	    			{
	    				case 0 : this.dropRandomSword(DivineForce.epicSword1.itemID, 1, 4);break;
	    				case 1 : this.dropRandomSword(DivineForce.epicSword2.itemID, 1, 4);break;
	    				case 2 : this.dropRandomSword(DivineForce.epicSword3.itemID, 1, 4);break;
	    				case 3 : this.dropRandomSword(DivineForce.epicSword4.itemID, 1, 4);break;
	    			}			
	    		}break;
	    	}
        }
    ...
    {
        return ((float)this.getEntityAttribute(SharedMonsterAttributes.maxHealth).getAttributeValue() * (1 + (float)this.dataWatcher.getWatchableObjectInt(20)/10)) * ((this.dataWatcher.getWatchableObjectInt(21) * 2F) - 1);
    }
    ...
    private int getArmSwingAnimationEnd()
    {
    	...
    	{
    		return (int)(((EntityPlayer)this).attackSpeed);
    	} else
    ...
    public void handleHealthUpdate(byte par1)
    {
        ...
        {
        	//moded
            this.limbSwingAmount = 0.2F;
            this.hurtResistantTime = this.maxHurtResistantTime;
            //moded
            this.hurtTime = this.maxHurtTime = 3;
            this.attackedAtYaw = 0.0F;
    ...
    public void onLivingUpdate()
    {
    	...
    	if(this.isStuned())
    	{
    		...
    		{
    			worldObj.spawnParticle("spell", this.posX, this.posY + this.height * 1.2F, this.posZ,  rand.nextFloat()/25 - rand.nextFloat()/25, 0.5F,  rand.nextFloat()/25 - rand.nextFloat()/25);
    		}
    		this.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 1, 50));
    	}
        ...
        if(this.damageBarAnim > this.getHealthPercentage())
        {
        	if(this.damageBarAnim - 1 == this.getHealthPercentage())
        	{
        		this.damageBarAnim -= 1F;
        	} else
        	{
        		this.damageBarAnim -= 2F;
        	}
        }
        else
        {
        	this.damageBarAnim = this.getHealthPercentage();
        }
    ...
    public void splashDamage(float damage, boolean isSpecial, boolean crit)
    {
        float distanceToPlayer = 0;
        if(Minecraft.getMinecraft().thePlayer != null)
        {
        	distanceToPlayer = this.getDistanceToEntity(Minecraft.getMinecraft().thePlayer)/5;
        	if(distanceToPlayer > 1.5F)
        	{
        		distanceToPlayer = 1.5F;
        	}
        }
        if(Minecraft.getMinecraft().thePlayer != null && Minecraft.getMinecraft().thePlayer.getDistanceToEntity(this) < 64)
        {
        	float critOffset;
        	float offset1 = 0;
        	float offset2 = 0;
        	...
        	if(crit)
        	{
        		offset1 = rand.nextFloat()/2 - rand.nextFloat()/2;
        		offset2 = rand.nextFloat()/4;
        		offset3 = rand.nextFloat()/2 - rand.nextFloat()/2;
        		critOffset = 1.2F;
        		Minecraft.getMinecraft().thePlayer.playSound("random.anvil_land", 0.2F, 1.0F);
        	} else
        	{
        		critOffset = 1F;
        	}
        	EntityDamageFX damageFX = new EntityDamageFX(Minecraft.getMinecraft().theWorld, posX + offset1, posY + offset2 + (Minecraft.getMinecraft().thePlayer.height - 0.6F + distanceToPlayer) * critOffset, posZ + offset3, 0, 0, 0, (int)damage, isSpecial, crit);                 
        	Minecraft.getMinecraft().effectRenderer.addEffect(damageFX);
        }
    }
    ...
    public float damageHealthBarAnim(float f)
    {
    	float f2 = this.getHealthPercentage();
    	if(f > f2)
    		f-= 0.5F;
    	return f;
    }
    ...
    public void setRarity(int par1)
    {
    ...
    public EntityItem dropRandomSword(int par1, int par2, int par3)
    {
    	ItemStack itemstack = new ItemStack(par1, par2, 0);
    	itemstack.setLevel(this.getLevel());
    	itemstack.setRarity(par3);  
    	itemstack.setType(rand.nextInt(11));
    	itemstack.setAttributePoints(((int)itemstack.getLevel() / 4.166666666666666F * itemstack.getRarity2() / 3F));
    	itemstack.setAttackDamage((float) (Math.ceil((itemstack.getLevel() / 1.5F * itemstack.getRarity2() / 3F))) + 3);
    	itemstack.setAttackSpeed((itemstack.getAttributePoints() * 2) * (float)itemstack.getType()/10);
    	itemstack.setCrit(itemstack.getAttributePoints() - (itemstack.getAttributePoints() * (float)itemstack.getType()/10));
    	return this.entityDropItem(itemstack, 0);
    }
    
    public void setStun(int par1)
    {
    	stun = par1;
    }
