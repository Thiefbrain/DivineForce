...
public class Minecraft implements IPlayerUsage
{
    ...
    private void sendClickBlockToController(int par1, boolean par2)
    {
        ...
        if (par1 != 0 || this.leftClickCounter <= 0)
        {
        	...
        	//moded
//			[        	
	        	else if (par2 && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == EnumMovingObjectType.ENTITY && par1 == 0 && this.thePlayer.attackTimer == 0)
	            {
	        		this.thePlayer.swingItem();
	                this.playerController.attackEntity(this.thePlayer, this.objectMouseOver.entityHit);   
	                this.thePlayer.attackTimer = this.thePlayer.attackSpeed;
	            }
//        	]
            else
            {
    ...
    public void clickMouse(int par1)
    {
        if (par1 != 0 || this.leftClickCounter <= 0)
        {
            ...
            {
            	//moded
                //if (par1 == 0 && this.thePlayer.attackTimer == 0)
                //{
                    //this.playerController.attackEntity(this.thePlayer, this.objectMouseOver.entityHit);
                //}
    ...
    private void displayTarget(int par1)
    {
    	if(thePlayer != null)
        ...
        double d = 100;
        objectMouseOver2 = renderViewEntity.rayTrace(d, par1);
        double d1 = d;
        Vec3 vec3d = renderViewEntity.getPosition(par1);

        if (objectMouseOver2 != null)
        {
            d1 = objectMouseOver2.hitVec.distanceTo(vec3d);
        }

        Vec3 vec3d1 = renderViewEntity.getLook(par1);
        Vec3 vec3d2 = vec3d.addVector(vec3d1.xCoord * d, vec3d1.yCoord * d, vec3d1.zCoord * d);
        theTarget = null;
        float f = 1.0F;
        java.util.List list = theWorld.getEntitiesWithinAABBExcludingEntity(renderViewEntity, renderViewEntity.boundingBox.addCoord(vec3d1.xCoord * d, vec3d1.yCoord * d, vec3d1.zCoord * d).expand(f, f, f));
        double d2 = d1;
        ...
        for (int i = 0; i < list.size(); i++)
        {
            ...
            if (axisalignedbb.isVecInside(vec3d))
            {
                if (0.0D < d2 || d2 == 0.0D)
                {
                    theTarget = (EntityLivingBase)entity;
                    d2 = 0.0D;
                }
    ...
    public void spellKeyBindings()
    {
    	...
    	{
    		if(thePlayer.spellBar[0] != null) thePlayer.spellBar[0].triggerSpell(this.thePlayer.username);
    	}
    	if(Keyboard.isKeyDown(Keyboard.KEY_R))
    	{
    		if(thePlayer.spellBar[1] != null) thePlayer.spellBar[1].triggerSpell(this.thePlayer.username);
    	}
    	if(Keyboard.isKeyDown(Keyboard.KEY_F))
    	{
    		if(thePlayer.spellBar[2] != null) thePlayer.spellBar[2].triggerSpell(this.thePlayer.username);
    	}
    	if(Keyboard.isKeyDown(Keyboard.KEY_X))
    	{
    		if(thePlayer.spellBar[3] != null) thePlayer.spellBar[3].triggerSpell(this.thePlayer.username);
    	}
    	if(Keyboard.isKeyDown(Keyboard.KEY_V))
    	{
    		if(thePlayer.spellBar[4] != null) thePlayer.spellBar[4].triggerSpell(this.thePlayer.username);
    	}
    	if(Keyboard.isKeyDown(Keyboard.KEY_B))
    	{
    		this.displayGuiScreen(new GuiSpells(thePlayer));
    	}*/
    }
}
